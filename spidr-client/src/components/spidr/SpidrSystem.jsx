...
const MOCK_NEWS = [
  { id: 'p1973', title: 'Patch 1.9.73 — Search & Media Hub', date: '2026-08-26', type: 'UPDATE', description: 'Every chat surface can now be searched, and the media in it can be browsed. Open the magnifier in a DM or group chat header and you get keyword search across that entire conversation, plus an Images tab that lays every picture ever sent into a thumbnail grid and a Links tab listing every URL with who posted it and when. Server channels already had keyword search through the Signal Tracker, so they gain just the Images and Links tabs beside it rather than a second search box. Search understands from:name — type from:alice cats and you get every message Alice sent containing cats, resolved to an indexed author lookup rather than a text scan. Under the hood this is built for scale rather than convenience: messages now compute has_images and has_links flags at WRITE time and store the extracted media and link URLs, and those flags are indexed. Opening the Images tab is an index hit no matter how much history a chat has, instead of scanning every message body looking for http. Keyword search hits the Mongo text index first for relevance ranking, then falls back to an escaped regex, because text indexes match whole words only and would never find hel inside hello. Existing history works with no migration — messages written before the flags existed have their metadata derived on read. Pasted image links count as images, so a picture shared as a URL still lands in the gallery. Every scope is membership-guarded: a conversation id is not a capability, and without those checks anyone could read any DM by guessing an id.' },
  { id: 'p1972', title: 'Patch 1.9.72 — #tags are unique, and can be words', date: '2026-08-26', type: 'UPDATE', description: 'Everyone appearing with the same #tag had a specific cause, and it was not the random generator — that was already producing 1.7 million combinations. Profiles created before the tag field existed (or through a write path that skipped the model hook) had NO tag at all, and the client papered over that with a hardcoded fallback of 0000. So every tagless user rendered as Name#0000 and they all looked identical. Three fixes: the fake fallback is gone, so a missing tag shows nothing rather than a lie; a boot-time backfill gives every legacy profile a real tag, derived deterministically from the user id so repeat runs do not churn tags; and generation now queries the database for a free tag instead of trusting randomness, retrying on collision with a safety counter so a popular name cannot spin forever. Tags can be WORDS now. Claim one in Settings under your name — Auxtin#vibes, Auxtin#spooky — with live availability checking as you type. Uniqueness is scoped to the name, so many people can be #vibes but only one can be Auxtin#vibes. Custom tags are 2 to 12 characters, letters and numbers only (spaces and symbols would break tags in URLs), case-folded so VIBES and vibes cannot become two different identities, and blocked from impersonating staff — admin, moderator, system, official and friends are reserved. Also closed a hole found on the way: the tag field was writable through the ordinary profile update endpoint, meaning a crafted request could hand someone any tag including a reserved one, bypassing every check. It is now server-route-only. Random tags also dropped the characters 0, o, 1, l and i, since tags get read aloud and typed by hand when adding friends.' },
  { id: 'p1971', title: 'Patch 1.9.71 — Bulletproof avatars everywhere', date: '2026-08-24', type: 'FIX', description: 'The Server Matrix already used the initials-fallback avatar, but seven other places in the app still rendered server icons with a raw image tag — the server landing header, the Nerve Center server rows, the Engagement Hub, and the mutual-servers chips on profiles. Those had graceful handling for a MISSING icon but nothing for a DEAD one, so an expired or deleted URL fell through to the app-wide broken-image net and showed a generic spider mark that tells you nothing about which server you are looking at. All four are converted now and degrade to the server\'s own initials instead. The avatar component gained a fallbackClassName so each surface keeps its established look rather than every server icon in the app snapping to one style — the Nerve Center and Engagement Hub tiles keep their red gradient, the Matrix keeps its glass tile — plus a letters option so small chips show one initial and larger tiles show two.' },
  { id: 'p1970', title: 'Patch 1.9.70 — QA sweep: callers, @everyone, wallpapers', date: '2026-08-22', type: 'UPDATE', description: 'Missed-call rows said You missed a call from Someone instead of naming the caller. Root cause: the server-side name resolver queried UserProfile for both display_name AND username — but username lives on the User model, not UserProfile. Any account without a custom display name resolved to nothing and fell through to the Someone fallback. It now checks both models, preferring the chosen display name, then the real account name. @everyone now actually works in group chats. The mention scanner treats everyone and here as reserved tokens and deliberately never expands them, which is correct for big server channels where one admin could generate thousands of notification rows — but group chats are small and private, so a ping there fans out to every member. Mentions are unioned so at everyone plus at alice does not notify alice twice. The at-mention autocomplete popup listing chat members already existed and is live in DMs, groups, and server channels. Custom chat backgrounds landed for both DMs and group chats. Group wallpapers are shared across the whole group and set from Group Settings; DM wallpapers are per-user, because a DM has no container document to hang a shared setting on — which also means each side of a conversation picks their own without overwriting the other. Six built-in gradients ship inline as data-URIs so they cost no network requests and can never 404, plus image upload. Every wallpaper renders under a dark scrim with a slight blur, because a bright image behind white message text is unreadable otherwise.' },
  { id: 'p1969', title: 'Patch 1.9.69 — One place for servers', date: '2026-08-22', type: 'UPDATE', description: 'Servers were rendering in two places at once — the vertical avatar strip at the bottom of the far-left rail AND the Server Matrix panel right beside it — so the eye had two competing places to switch context. The rail strip is gone. The far-left column is now strictly global routing (Home, Friends, the Servers toggle, Bot Lab, Settings) and servers live exclusively in the Matrix panel, where there is actually room for full names, member counts, and search. Create Server moved into the Matrix panel with matching row geometry, since the rail strip that used to own that button no longer exists — it would otherwise have been stranded. Server icons in the Matrix are now bulletproof: a new ServerAvatar component catches dead image URLs and degrades to a clean initials tile, so Anime Haven with a broken icon shows a tidy AN rather than the browser\'s torn-page glyph. That is a deliberate step up from the app-wide broken-image net, which substitutes a generic spider mark — correct as a default, but it tells you nothing about WHICH server you are looking at. The tile also resets itself when a server uploads a new icon, so it will not stay stuck on initials. The collapse chevron beside the search bar was already wired and still works. Cleaned up after the removal: the imports and the onCreateServer prop the deleted strip left behind are gone rather than lingering as debris that suggests the strip is still there.' },
