...
const MOCK_NEWS = [
  { id: 'p1939', title: 'Patch 1.9.39 — AI identity polish + Jump Back In everywhere', date: '2026-07-09', type: 'FIX', description: 'Spidr AI finally looks like Spidr everywhere. The AI panel\'s assistant avatar dropped the solid red circle with the geometric logo for the real transparent mascot — the art floats free with a soft red glow, no background tile, no square crop. The persona tabs shed their OS emojis for proper vector iconography: Spidr wears its bespoke spider sigil, Rogue a purple skull, Ghost the ghost mark, Hype a flame, Sage a leaf, Noir the venetian mask — and Catch Me Up gets a golden bolt. Each tinted to its persona. Jump Back In (recent DMs + group chats) now shows on EVERY screen size: the right rail stays on wide desktop displays, and below that width the same panel renders in the main column — one shared component, so the two mounts can never drift apart. Phone, tablet, laptop, ultrawide: your conversations are one tap from home everywhere. Also re-verified end-to-end this patch: Save to Collection on THE WEB (picker state, mutations, schema, route ownership, and error toasts all confirmed wired), the Apex badge button in Settings, and the mascot as the AI\'s avatar across chat surfaces — if any of these look wrong on your screen, you are running a previous deploy; check this patch number appears in Spidr System after updating.' },
  { id: 'p1938', title: 'Patch 1.9.38 — Real brand art everywhere', date: '2026-07-09', type: 'UPDATE', description: 'Three logo drop-ins across the app. The homepage welcome slab now wears the real Spidr mascot art — the same chrome ring, but the flat geometric SpiderLogo swapped for the full illustrated crest with the red fangs and glowing eyes. Spidr AI\'s avatar upgrades everywhere it appears (voice tiles, chat bubbles, message headers, DM cards, server rooms) via the single SPIDR_AI_AVATAR export the AI already routes through — one file swap, six surfaces updated. And the Apex button in Settings drops the mini icon-plus-text combo for the full APEX TIER shield badge as the button itself, with a soft gold hover aura. No mocks, no filler — the crest art you sent is the button.' },
  { id: 'p1937', title: 'Patch 1.9.37 — Real names in Jump Back In', date: '2026-07-08', type: 'FIX', description: 'The "Node" labels in Jump Back In were not mocked — they were real DMs whose sender name resolution had run out of fallbacks. Root cause: DMs YOU send stamp sender_name but never stamped recipient_name (a schema gap from before recipient denormalization existed), so any conversation on the list where you sent the last message had no name to show and hit the fallback string. Fixed at both send call-sites — outgoing DMs now stamp recipient_name and recipient_avatar from the active conversation, matching the sender denorm pattern. Jump Back In also gained a real-time backfill: for old rows still missing recipient info, it now resolves the friend from your friend list (name + avatar) instead of showing the fallback. Old "Node" rows will fix themselves on your next reply to that thread, or immediately for anyone in your friend list.' },
