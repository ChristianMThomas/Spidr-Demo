    ...
    <div className="flex-1 flex bg-gradient-to-br from-zinc-950 via-black to-red-950/20">
      ...
      {uploadedFile && (
        <VideoStudio open={showUpload} onClose={() => { setShowUpload(false); setUploadedFile(null); }}
          ...
          onPublish={async (d) => { await entities.Clip.create(d); queryClient.invalidateQueries(['clips']); queryClient.invalidateQueries(['algo-feed']); toast.success('Strand deployed!'); }} />
      )}
      {editingClip && (
        <VideoStudio open={!!editingClip} onClose={() => setEditingClip(null)} videoFile={null} currentUser={currentUser} initialClip={editingClip}
          onPublish={async (d) => { await entities.Clip.update(editingClip.id, d); queryClient.invalidateQueries(['clips']); toast.success('Updated!'); setEditingClip(null); }} />
      )}
    </div>
  );
}
...
function ClipViewer({ clips, currentUser, queryClient, onEditClip, feedPersonalized }) {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { toast.success('Saved!'); queryClient.invalidateQueries(['collections']); },
  });
...
function CollectionsView({ collections, selectedCollection, onSelectCollection, currentUser, queryClient, allClips }) {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(['collections']); toast.success('Created!'); setNewName(''); setShowNew(false); },
  });
