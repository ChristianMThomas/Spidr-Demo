...
export default function MyBotsTab({ currentUser }) {
  ...
  const { data: myBots = [], isLoading } = useQuery({
    ...
    queryFn: () => entities.CustomBot.filter({ author_id: currentUser?.id }),
  ...
  const createBotMutation = useMutation({
    mutationFn: async (botData) => {
      let avatar_url = '';
      if (avatarFile) {
        const result = await integrations.Core.UploadFile({ file: avatarFile });
  ...
  const deleteBotMutation = useMutation({
    mutationFn: (id) => entities.CustomBot.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-bots'] });
  ...
  const runSafetyScan = async (botId, name, description) => {
    setScanning(botId);
    await entities.CustomBot.update(botId, { status: 'scanning' });
    queryClient.invalidateQueries({ queryKey: ['my-bots'] });
...
Return your analysis.`,
    ...
    await entities.CustomBot.update(botId, {
      status: newStatus,
  ...
  const togglePublic = async (bot) => {
    ...
    await entities.CustomBot.update(bot.id, { is_public: !bot.is_public });
    queryClient.invalidateQueries({ queryKey: ['my-bots'] });
