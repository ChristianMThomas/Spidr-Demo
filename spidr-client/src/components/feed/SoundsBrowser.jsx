...
import { Search, TrendingUp, Upload, Play, Pause, Disc3, Music, Loader2, User, Bookmark, X, Globe } from 'lucide-react';
...
export default function SoundsBrowser({ currentUser }) {
  ...
  const { data: allTracks = [], isLoading } = useQuery({
    ...
    queryFn: () => entities.AudioTrack.list('-use_count', 100),
    staleTime: 30000,
  });
  ...
  const { data: clips = [] } = useQuery({
    queryKey: ['clips'],
    queryFn: () => entities.Clip.list('-created_date', 100),
    staleTime: 30000,
  });
  ...
  const getDisplayTracks = () => {
    ...
    switch (activeTab) {
      ...
      case 'originals':
        return tracks.filter(t => t.is_original || t.url);
      case 'saved':
  ...
  const handleUpload = async (e) => {
    ...
    await entities.AudioTrack.create({
  ...
  if (selectedAudio) {
    return (
      <FrequencyArchive
        ...
        onUseSound={(track) => {
          // Let the user know they can use this in VideoStudio
          toast.success(`"${track.title}" ready — use it when posting a clip!`);
            ...
            {displayTracks.map((track, idx) => (
              ...
              >
                ...
                <div className="text-[10px] text-gray-500 font-mono flex items-center gap-2 mt-0.5">
                  ...
                  <span>•</span>
                  <span className="text-[#FF3333] font-bold">{formatCount(clipCountMap[track.id] || track.use_count || 0)} videos</span>
                  {track.save_count > 0 && (
                    <>
                      <span>•</span>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <div className="opacity-0 group-hover:opacity-100 text-[9px] font-black uppercase text-[#FF3333] bg-[#FF3333]/10 border border-[#FF3333]/30 px-2 py-1 rounded-lg transition-all">
                    See Videos →
                  </div>
                </div>
              </motion.div>
