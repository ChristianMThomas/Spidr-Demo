const { Resend } = require('resend');

const resend = new Resend(process.env.RESEND_API_KEY);
const FROM = process.env.FROM_EMAIL || 'noreply@spidrapp.com';

// Base64 logo (keep as-is — already embedded)
const LOGO_B64 = 'data:image/png;base64,';

async function sendBetaConfirmEmail(toEmail, fullName) {
  const firstName = fullName.trim().split(' ')[0] || 'there';

  const listRows = [
    'Early access before the public launch',
    'A direct line to share feedback with our team',
    'First look at every feature we ship',
  ].map(text => `
    <tr>
      <td style="padding:5px 0 5px 0;">
        <table role="presentation" border="0" cellpadding="0" cellspacing="0">
          <tr>
            <td width="18" style="vertical-align:top;padding-top:3px;">
              <span style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;font-size:14px;color:#dc2626;font-weight:700;line-height:1;">›</span>
            </td>
            <td>
              <p style="margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;font-size:14px;line-height:1.55;color:#52525b;">
                <span class="txt-secondary" style="color:#52525b;">${text}</span>
              </p>
            </td>
          </tr>
        </table>
      </td>
    </tr>`).join('');

  const html = `<!DOCTYPE html>
<html lang="en" xmlns:v="urn:schemas-microsoft-com:vml" xmlns:o="urn:schemas-microsoft-com:office:office">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <meta name="color-scheme" content="light dark">
  <meta name="supported-color-schemes" content="light dark">
  <!--[if gte mso 9]><xml><o:OfficeDocumentSettings><o:PixelsPerInch>96</o:PixelsPerInch><o:AllowPNG/></o:OfficeDocumentSettings></xml><![endif]-->
  <title>You're on the Spidr beta list.</title>
  <style type="text/css">
    body, #root { margin: 0; padding: 0; width: 100%; }
    img { border: 0; outline: none; text-decoration: none; display: block; }
    a { color: #dc2626; }

    /* Mobile */
    @media only screen and (max-width: 600px) {
      .wrap { width: 100% !important; }
      .cell-pad { padding: 28px 20px !important; }
      .headline { font-size: 22px !important; }
    }

    /* Dark mode */
    @media (prefers-color-scheme: dark) {
      body, #root, .bg-page    { background-color: #09090b !important; }
      .bg-card                 { background-color: #18181b !important; }
      .bg-inset                { background-color: #09090b !important; border-color: #27272a !important; }
      .bg-header               { background-color: #000000 !important; }
      .txt-primary             { color: #fafafa !important; }
      .txt-secondary           { color: #a1a1aa !important; }
      .txt-muted               { color: #71717a !important; }
      .divider-cell            { background-color: #27272a !important; }
      .footer-cell             { background-color: #18181b !important; }
    }
  </style>
</head>
<body style="margin:0;padding:0;background-color:#f4f4f5;" bgcolor="#f4f4f5">

<table id="root" role="presentation" width="100%" cellpadding="0" cellspacing="0" class="bg-page" bgcolor="#f4f4f5" style="background-color:#f4f4f5;">
  <tr>
    <td align="center" style="padding:48px 16px 48px;">

      <table role="presentation" class="wrap" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;">

        <!-- ── HEADER ─────────────────────────────────────────────── -->
        <tr>
          <td align="center" class="bg-header" bgcolor="#0a0a0a"
              style="background-color:#0a0a0a;padding:30px 40px 28px;border-radius:16px 16px 0 0;">
            <p style="margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                      font-size:30px;font-weight:900;letter-spacing:-1px;color:#ffffff;line-height:1;">
              SPID<span style="color:#dc2626;">R</span>
            </p>
            <p style="margin:6px 0 0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                      font-size:10px;font-weight:600;letter-spacing:4px;text-transform:uppercase;color:#3f3f46;">
              BETA ACCESS
            </p>
          </td>
        </tr>

        <!-- ── BODY ──────────────────────────────────────────────── -->
        <tr>
          <td class="bg-card cell-pad" bgcolor="#ffffff"
              style="background-color:#ffffff;padding:36px 40px 32px;">

            <!-- Eyebrow -->
            <p style="margin:0 0 10px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                      font-size:11px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:#dc2626;">
              Confirmed
            </p>

            <!-- Headline -->
            <p class="headline txt-primary"
               style="margin:0 0 14px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                      font-size:26px;font-weight:800;color:#09090b;line-height:1.2;">
              You're on the list, ${firstName}.
            </p>

            <!-- Body copy -->
            <p class="txt-secondary"
               style="margin:0 0 28px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                      font-size:15px;line-height:1.65;color:#52525b;">
              Your spot in the Spidr closed beta is saved. We'll reach out when
              access opens — no action needed on your end.
            </p>

            <!-- Registered email pill -->
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
              <tr>
                <td class="bg-inset" bgcolor="#f4f4f5"
                    style="background-color:#f4f4f5;border-radius:10px;padding:14px 18px;
                           border:1px solid #e4e4e7;">
                  <p class="txt-muted"
                     style="margin:0 0 4px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                            font-size:10px;font-weight:600;letter-spacing:2px;text-transform:uppercase;color:#a1a1aa;">
                    Registered as
                  </p>
                  <p style="margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                            font-size:15px;font-weight:600;color:#dc2626;word-break:break-all;">
                    ${toEmail}
                  </p>
                </td>
              </tr>
            </table>

            <!-- What to expect -->
            <p class="txt-primary"
               style="margin:0 0 12px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                      font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#09090b;">
              What to expect
            </p>
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:32px;">
              ${listRows}
            </table>

            <!-- CTA button -->
            <table role="presentation" border="0" cellpadding="0" cellspacing="0">
              <tr>
                <td align="center" bgcolor="#dc2626"
                    style="background-color:#dc2626;border-radius:8px;mso-padding-alt:0;">
                  <!--[if mso]><v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="https://spidrapp.com" style="height:46px;v-text-anchor:middle;width:200px;" arcsize="17%" stroke="f" fillcolor="#dc2626"><w:anchorlock/><center style="color:#ffffff;font-family:Arial,sans-serif;font-size:15px;font-weight:bold;">Visit spidrapp.com</center></v:roundrect><![endif]-->
                  <!--[if !mso]><!-->
                  <a href="https://spidrapp.com" target="_blank"
                     style="display:inline-block;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                            font-size:15px;font-weight:700;color:#ffffff;text-decoration:none;
                            padding:13px 28px;border-radius:8px;mso-hide:all;">
                    Visit spidrapp.com &rsaquo;
                  </a>
                  <!--<![endif]-->
                </td>
              </tr>
            </table>

          </td>
        </tr>

        <!-- ── DIVIDER ────────────────────────────────────────────── -->
        <tr>
          <td class="bg-card" bgcolor="#ffffff" style="background-color:#ffffff;padding:0 40px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td class="divider-cell" height="1" bgcolor="#e4e4e7"
                    style="background-color:#e4e4e7;font-size:1px;line-height:1px;">&nbsp;</td>
              </tr>
            </table>
          </td>
        </tr>

        <!-- ── FOOTER ─────────────────────────────────────────────── -->
        <tr>
          <td class="bg-card footer-cell cell-pad" bgcolor="#ffffff"
              style="background-color:#ffffff;padding:22px 40px 30px;border-radius:0 0 16px 16px;">

            <!-- Signature row -->
            <table role="presentation" border="0" cellpadding="0" cellspacing="0" style="margin-bottom:18px;">
              <tr>
                <td width="44" style="vertical-align:middle;padding-right:14px;border-right:2px solid #dc2626;">
                  <img src="${LOGO_B64}" alt="Spidr" width="40" height="40"
                       style="display:block;width:40px;height:40px;border-radius:8px;" />
                </td>
                <td style="vertical-align:middle;padding-left:14px;">
                  <p style="margin:0 0 2px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                            font-size:14px;font-weight:800;letter-spacing:-0.3px;color:#09090b;" class="txt-primary">
                    SPID<span style="color:#dc2626;">R</span> Beta Team
                  </p>
                  <p style="margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                            font-size:11px;color:#a78bfa;font-weight:600;letter-spacing:1.5px;text-transform:uppercase;">
                    Enter The Chrysalis
                  </p>
                </td>
              </tr>
            </table>

            <!-- Links -->
            <p style="margin:0 0 14px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                      font-size:12px;color:#a1a1aa;">
              <a href="https://spidrapp.com" style="color:#a1a1aa;text-decoration:none;">spidrapp.com</a>
              &nbsp;&middot;&nbsp;
              <a href="mailto:noreply@spidrapp.com" style="color:#a1a1aa;text-decoration:none;">noreply@spidrapp.com</a>
            </p>

            <!-- Legal -->
            <p class="txt-muted"
               style="margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;
                      font-size:12px;line-height:1.6;color:#a1a1aa;">
              You're receiving this because you signed up at spidrapp.com.<br>
              This is an automated message &mdash; please do not reply directly.
            </p>

          </td>
        </tr>

      </table>
    </td>
  </tr>
</table>

</body>
</html>`;

  if (process.env.NODE_ENV !== 'production' && !process.env.RESEND_API_KEY) {
    console.log(`\n📧 [DEV] Beta confirm email → ${toEmail} (${fullName})\n`);
    return;
  }

  await resend.emails.send({
    from: `Spidr <${FROM}>`,
    to: toEmail,
    subject: "You're on the Spidr beta list.",
    html,
  });
}

module.exports = { sendBetaConfirmEmail };
