...
export default function ModuleNexus({ currentUser }) {
  ...
  const { data: modules = [], isLoading } = useQuery({
    queryKey: ['modules'],
    queryFn: () => base44.entities.Module.list('-install_count', 200),
  });

  // For discover tab, only show approved modules
  const approvedModules = modules.filter(m => m.status === 'approved');

  const { data: installed = [] } = useQuery({
    queryKey: ['installed-modules', currentUser?.id],
    queryFn: () => base44.entities.InstalledModule.filter({ user_id: currentUser?.id }),
  ...
  const installMutation = useMutation({
    mutationFn: async (moduleId) => {
      ...
      await base44.entities.InstalledModule.create({ user_id: currentUser.id, module_id: moduleId });
      const mod = modules.find(m => m.id === moduleId);
      if (mod) await base44.entities.Module.update(moduleId, { install_count: (mod.install_count || 0) + 1 });
    },
    onSuccess: () => {
  ...
  const uninstallMutation = useMutation({
    mutationFn: async (moduleId) => {
      ...
      if (record) await base44.entities.InstalledModule.delete(record.id);
      const mod = modules.find(m => m.id === moduleId);
      if (mod) await base44.entities.Module.update(moduleId, { install_count: Math.max(0, (mod.install_count || 1) - 1) });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['installed-modules', currentUser?.id] });
      queryClient.invalidateQueries({ queryKey: ['modules'] });
  ...
  const reportMutation = useMutation({
    mutationFn: async ({ moduleId, reason }) => {
      ...
      await base44.entities.Module.update(moduleId, update);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['modules'] });
