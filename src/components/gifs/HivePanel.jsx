...
export default function HivePanel({ currentUser, search }) {
  ...
  const { data: assets = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.CommunityAsset.list('-created_date', 100),
  });
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {filtered.map((asset) => {
            ...
            return (
              ...
              >
                {/* Asset */}
                <div className="aspect-square relative overflow-hidden cursor-pointer" onClick={() => copyAsset(asset.file_url)}>
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-2.5">
                    <div className="flex gap-1.5 w-full">
                      <button onClick={(e) => { e.stopPropagation(); copyAsset(asset.file_url); }}
