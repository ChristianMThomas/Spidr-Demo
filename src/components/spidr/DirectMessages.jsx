...
import { MoreVertical, Phone, Video, Ghost, Archive } from 'lucide-react';
...
export default function DirectMessages({ conversation, currentUser, onBack, recipientId, conversationId, onVoiceJoin, onVoiceLeave, onMinimizeCall }) {
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: () => base44.entities.DirectMessage.filter({ conversation_id: activeConversationId }),
  ...
  const { data: recipientProfile } = useQuery({
    ...
    queryFn: async () => {
      const profiles = await base44.entities.UserProfile.filter({ user_id: activeRecipientId });
      return profiles[0];
    },
  ...
  const { data: voiceSessions = [] } = useQuery({
    ...
    queryFn: () => base44.entities.VoiceSession.filter({ channel_id: activeConversationId }),
