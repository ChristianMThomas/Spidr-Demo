...
export default function DMsSidebar({ currentUser, onSelectConversation, activeConversationId, onOpenAddFriend }) {
  ...
  const { data: allMessages = [] } = useQuery({
    ...
    queryFn: async () => {
      const sent = await base44.entities.DirectMessage.filter({ sender_id: currentUser?.id });
      const received = await base44.entities.DirectMessage.filter({ recipient_id: currentUser?.id });
...
function EmptyState({ isHoveringButton, onButtonHover, onOpenAddFriend, onSpiderClick, showGlitch }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 px-4">
      ...
      >
        <motion.img
          src="https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/698bbf4e09bd37d0bd1c3b99/3c28f0fc3_MainLogo.png"
          alt="Sleeping Spider"
