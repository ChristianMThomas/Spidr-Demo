...
import { Activity, UserPlus, MessageSquare, Check, X, Bell } from 'lucide-react';
...
export default function PulseDeck({ currentUser, onNavigateDM, onNavigateServer }) {
  ...
  const acceptFriendMutation = useMutation({
    mutationFn: async (friendRecord) => {
      await base44.entities.Friend.update(friendRecord.id, { status: 'accepted' });
      // Also update the reverse record
      const reverseRecords = await base44.entities.Friend.filter({ user_id: friendRecord.friend_id, friend_id: friendRecord.user_id });
      if (reverseRecords.length > 0) {
        await base44.entities.Friend.update(reverseRecords[0].id, { status: 'accepted' });
      }
    },
    onSuccess: () => {
  ...
  const denyFriendMutation = useMutation({
    mutationFn: async (friendRecord) => {
      await base44.entities.Friend.delete(friendRecord.id);
      const reverseRecords = await base44.entities.Friend.filter({ user_id: friendRecord.friend_id, friend_id: friendRecord.user_id });
      if (reverseRecords.length > 0) {
        await base44.entities.Friend.delete(reverseRecords[0].id);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['friend-requests-pulse'] });
