...
import { Heart, Reply, Image as ImageIcon, Loader2, X } from 'lucide-react';
...
function CommentForm({ clipId, currentUser, parentCommentId = null, onSuccess, serverEmojis, compact = false }) {
  ...
  const createCommentMutation = useMutation({
    mutationFn: async (commentData) => base44.entities.Comment.create(commentData),
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      setMediaFiles([...mediaFiles, ...results.map(r => r.file_url)]);
    } catch (error) {
...
export default function RichComments({ clipId, currentUser }) {
  const { data: comments = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.Comment.filter({ clip_id: clipId }),
  });
