    ...
    >
      ...
      >
        ...
        <div className="w-14 h-14 rounded-full border-2 border-[#0a0a0a] overflow-hidden bg-zinc-900">
          <img 
            src={friend.friend_avatar || 'https://via.placeholder.com/56'} 
            alt={displayName} 
...
export default function QuickHeads({ currentUser, onOpenDM, onOpenGroup }) {
  ...
  const { data: allGroupMessages = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!currentUser?.id) return [];
      const msgs = await base44.entities.GroupChatMessage.list('-created_date', 50);
      return msgs;
    },
    enabled: !!currentUser?.id,
    staleTime: 20000,
  ...
  const { data: groups = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!currentUser?.id) return [];
      const allGroups = await base44.entities.GroupChat.list('-created_date', 50);
