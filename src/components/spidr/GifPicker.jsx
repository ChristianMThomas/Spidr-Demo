...
import { Search, Loader2, Flame, Laugh, Heart, Gamepad2, Star, Cat, PartyPopper, Sparkles, Globe } from 'lucide-react';
...
export default function GifPicker({ onGifSelect }) {
  ...
  const { data: communityGifs = [] } = useQuery({
    ...
    queryFn: () => base44.entities.CommunityAsset.filter({ type: 'gif', is_public: true }, '-created_date', 50),
    staleTime: 60000,
  });
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {communityGifs.slice(0, 6).map(gif => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    onClick={() => onGifSelect(gif.file_url)}
                    className="aspect-square rounded-lg overflow-hidden bg-zinc-800 border border-white/5 hover:border-[#FF3333]/30"
                  >
                    <img src={gif.file_url} alt={gif.name} className="w-full h-full object-cover" loading="lazy" />
                  </motion.button>
                ))}
              </div>
            </div>
