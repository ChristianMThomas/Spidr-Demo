...
export function useGlobalEmojis() {
  const { data: allServers = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Server.list('-created_date', 200),
    staleTime: 60000,
  });

  const { data: communityAssets = [] } = useQuery({
    queryKey: ['community-assets-emojis'],
    queryFn: () => base44.entities.CommunityAsset.filter({ type: 'emoji', is_public: true }, '-created_date', 200),
    staleTime: 60000,
  });
  ...
  const emojiMap = React.useMemo(() => {
    ...
    communityAssets.forEach(asset => {
      if (asset.name && asset.file_url) {
        const key = `:${asset.name}:`;
        if (!map[key]) {
          map[key] = { url: asset.file_url, name: asset.name, animated: false, server: 'Community' };
        }
      }
    });
    return map;
