...
export default function FriendsPanel({ currentUser, onVoiceJoin, onVoiceLeave, onMinimizeCall, pendingDM, onPendingDMHandled }) {
  ...
  const handleAddFriend = async () => {
    ...
    // Parse Name#1234 format
    const match = input.match(/^(.+)#(\d{4})$/);
    if (!match) {
      toast.error('Invalid format. Use: Name#1234');
      return;
    }

    const [, displayName, discriminator] = match;
    
    // Find user by display_name and discriminator
    const targetProfile = profiles.find(
      p => p.display_name === displayName && p.discriminator === discriminator
    );
    ...
    const existing = friends.find(f => f.friend_id === targetProfile.user_id);
    if (existing) {
    ...
    try {
      ...
      await base44.entities.Friend.create({
        ...
        friend_name: currentUser?.display_name,
        friend_discriminator: currentUser?.discriminator,
                    ...
                    <div className="flex gap-2">
                      <Button 
                        ...
                        onClick={async () => {
                          ...
                          const outgoing = await base44.entities.Friend.filter({ user_id: friend.friend_id, friend_id: currentUser?.id });
                          if (outgoing[0]) {
...
function FriendCard({ friend, profile, currentUser, onViewProfile, queryClient, handleOpenDM, unreadInfo }) {
  ...
  const handleRemoveFriend = async () => {
    ...
    const reverse = await base44.entities.Friend.filter({ user_id: friend.friend_id, friend_id: currentUser?.id });
    if (reverse[0]) {
