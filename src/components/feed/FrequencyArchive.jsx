...
import { Play, Pause, Bookmark, Radio, Activity, ArrowLeft, Music } from 'lucide-react';
import { toast } from 'sonner';

export default function FrequencyArchive({ audioTrack, onClose, currentUser, onClipClick }) {
  ...
  const { data: clipsUsingAudio = [] } = useQuery({
    ...
    queryFn: () => base44.entities.Clip.filter({ audio_id: audioTrack?.id }),
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (isSaved) {
        ...
        if (existing) await base44.entities.SavedAudio.delete(existing.id);
      } else {
        await base44.entities.SavedAudio.create({ user_id: currentUser?.id, audio_id: audioTrack?.id });
        await base44.entities.AudioTrack.update(audioTrack.id, { save_count: (audioTrack.save_count || 0) + 1 });
      }
    },
    onSuccess: () => {
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              >
                ...
                {isSaved ? 'In Database' : 'Extract Frequency'}
              </button>
            </div>
          </div>
        </div>
