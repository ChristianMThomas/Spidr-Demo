...
export default function Home() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      default:
        return <HomeContent currentUser={currentUser} setActiveTab={setActiveTab} setSelectedServerId={setSelectedServerId} onNavigateToDM={handleNavigateToDM} />;
    }
  };
        ...
        <ActiveCallTether 
          ...
          onDisconnect={async () => {
            ...
            if (activeCall.type === 'server') {
              ...
              if (sessions[0]) {
                await base44.entities.VoiceSession.delete(sessions[0].id);
              }
            } else if (activeCall.type === 'group') {
              const sessions = await base44.entities.VoiceSession.filter({
...
function HomeContent({ currentUser, setActiveTab, setSelectedServerId, onNavigateToDM }) {
  const { data: servers = [] } = useQuery({
    queryKey: ['servers'],
    queryFn: () => base44.entities.Server.list('-created_date', 5),
    staleTime: 60000,
  });
      ...
      <div className="flex gap-6 p-6 max-w-[1400px] mx-auto">
        ...
        <div className="flex-1 min-w-0 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setActiveTab('friends')}
