...
s.post('save', async function (doc) {
  ...
  try {
    ...
    if (doc.content && doc.content.includes('@')) {
      ...
      // mention scanner treats these as reserved tokens and never expands
      // them — correct for large server channels, where one admin could
      // generate thousands of rows — but group chats are small and private,
      // so the broadcast is both wanted and cheap. Unioned into mentionedIds
      // so "@everyone and @alice" doesn't notify alice twice.
      const isEveryonePing = /@(everyone|here)\b/i.test(doc.content || '');
      if (isEveryonePing) {
        for (const uid of memberIds) {
          if (String(uid) !== String(doc.user_id)) mentionedIds.add(String(uid));
        }
      }

      const recipients = isEveryonePing
        ? [...mentionedIds].map(uid => ({ user_id: uid }))
        : mentioned;
