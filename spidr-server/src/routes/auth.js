...
router.post('/override-request', async (req, res) => {
  try {
    ...
    try { await sendOTPEmail(email, otp, 'verify'); } catch (e) { console.warn('Reset email failed:', e.message); }
    res.json({ method: 'email', message: 'Reset code sent to your email' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.post('/override-verify', async (req, res) => {
  try {
...
router.post('/override-confirm', async (req, res) => {
  try {
