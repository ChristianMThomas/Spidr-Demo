...
router.get('/search', authMW, async (req, res) => {
  ...
  try {
    ...
    if (token) {
      // Real Spotify search
      const qs     = new URLSearchParams({ q: q.trim(), type: 'track', limit: Math.min(parseInt(limit), 20) });
      const result = await fetch(`https://api.spotify.com/v1/search?${qs}`, {
