...
const NEWS = [
  {
    id: 'p1964',
    title: 'Patch 1.9.64 — iOS push notifications live end-to-end (RNFB v26 shim + firebase-admin v12 fix)',
    date: '2026-08-20',
    type: 'FIX',
    description: 'iOS push notifications finally light up the lock screen — the first time a backgrounded phone actually receives a Spidr DM banner. Three real bugs stacked on top of each other were hiding the finish line: @react-native-firebase/messaging v22+ dropped its callable default export in favor of a modular API, so every require(...).default at mobile boot returned undefined and the module silently failed to load with no exception in the safeRequire catch; firebase-admin v12+ hoisted cert to the top level and moved messaging into its own firebase-admin/messaging subpath, so admin.credential.cert(creds) and admin.messaging().sendEachForMulticast(...) both crashed the server the first time we tried to send; and even after both API shifts were patched, Firebase itself was refusing every push with messaging/third-party-auth-error because the APNs .p8 authentication key had never been uploaded to Cloud Messaging with its Key ID + Team ID. Fixed by wrapping RNFB\'s modular exports in a callable shim inside nativeCalls.ts so the existing messaging().foo(...) shape across callManager keeps working — getToken, onMessage, onNotificationOpenedApp, setBackgroundMessageHandler, hasPermission, requestPermission, registerDeviceForRemoteMessages, getAPNSToken, and the AuthorizationStatus / NotificationAndroidPriority constants all pass through the shim to the underlying modular functions with the messaging instance injected as the first arg — and by rewriting mobile/index.js to call fbMsg.setBackgroundMessageHandler(instance, cb) directly so the pre-React FCM background handler that CallKeep depends on actually registers now instead of silently throwing at undefined() and being swallowed by the outer try/catch. Server-side push.js and scripts/send-test-push.js swap admin.credential.cert for admin.cert and pull getMessaging from firebase-admin/messaging, and push.js re-attaches admin.messaging = () => getMessaging() so every downstream sendEachForMulticast caller keeps working without touching the DM / friend-request / call route handlers. Diagnostic logs added along the way — a one-line native-module inventory at callManager init (webrtc / callkeep / messaging / expoNotifications booleans) and a [nativeCalls] warn on any require() throw — so "module missing from binary" vs "module crashed while initializing" can be told apart from a single log paste. Also picked up during the same pass: a new mobile SpidrWebMatrix component that ports the web home page\'s Jump Back In / Pinned panel to the mobile home tab alongside RecentServers with the ActivityFeed collapsible below, and the missing iOS app.json icon/splash keys. NOTE: production DM push still requires FIREBASE_SERVICE_ACCOUNT to be set on the Railway spidr-server service — the code is all wired but the env var lives per-environment.',
  },
  {
    id: 'p1963',
