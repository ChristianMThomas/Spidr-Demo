...
app.use(rateLimit({ windowMs: 15 * 60 * 1000, max: 1000, skip: (req) => req.path.startsWith('/uploads') }));
...
function startSocketIO(withRedis) {
  ...
  // Add nerve-center room handler
  io.on('connection', (socket) => {
    socket.on('join:nerve-center', () => {
      // Only allow if the socket's user is admin (checked on frontend already)
...
// Try Redis; if it errors within 2 seconds, fall back to no-Redis mode
...
testRedis.on('error', () => {
  if (!redisReady) {
    redisReady = true; // prevent double-start
    testRedis.quit().catch(() => {});
    startSocketIO(false);
  }
});

// Guarantee startup even if Redis never responds
setTimeout(() => {
  if (!redisReady) {
    redisReady = true;
    startSocketIO(false);
